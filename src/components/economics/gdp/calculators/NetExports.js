...
const NetExports = () => {
  ...
  const [valueOfExports, setValueExports] = useState('');
  const [valueOfImports, setValueImports] = useState('');
  const [currency, setCurrency] = useState('ZAR');
  const [solution, setSolution] = useState(0);
  //variables
  const variableName1 = 'Value of Exports';
  const variableName2 = 'Value of Imports';
  ...
  const onChangeHandler = (variableName, value) => {
    if (variableName === variableName1) {
      setValueExports(value);
    }
    if (variableName === variableName2) {
      setValueImports(value);
    }
  };
  const onACHandler = () => {
    setValueExports('');
