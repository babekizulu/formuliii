...
const {atcfoFormula} = new FirmCashFlowStatementFormulae();

const ATCFOCalculator = () => {
    //state
    const [selected, setSelected] = useState('rand');
    const [earningsBITDA, setEarningsBITDA] = useState('');
    const [cashTaxPayments, setCashTaxPayments] = useState('');
    const [solution, setSolution] = useState(0);
    const [solutionUM, setSolutionUM] = useState('ZAR');
    //vnames & subject tag
    const vname1 = `Earnings B.I.T.D.A`;
    const vname2 = 'Cash Tax Payments';
...
export default ATCFOCalculator;