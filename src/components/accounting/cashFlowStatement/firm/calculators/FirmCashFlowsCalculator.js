...
const {cashFlowsFormula} = new FirmCashFlowStatementFormulae();

const FirmCashFlowsCalculator = () => {
    //state
    const [selected, setSelected] = useState('rand');
    const [afterTaxCashFlows, setAfterTaxCashFlows] = useState('');
    const [INOWC, setINOWC] = useState('');
    const [IFAOA, setIFAOA] = useState('');
    const [solution, setSolution] = useState(0);
    const [solutionUM, setSolutionUM] = useState('ZAR');
    //vnames & subject
    const vname1 = 'After-Tax Cash Flows';
    const vname2 = 'Investment in Net Operating Working Capital';
    const vname3 = 'Investment in Fixed Assets & Other Assets';
  ...
  useEffect(() => {
    ...
    const solutionHandler = () => {
        const solution = cashFlowsFormula(
            afterTaxCashFlows,
            INOWC,
            IFAOA
        );
    ...
    return (
        <div className="calculator-container">
            <BackBtn 
            prevUrl='/accounting/cash-flow-statement/firm'
            />
            <CurrencySelector
...
export default FirmCashFlowsCalculator;