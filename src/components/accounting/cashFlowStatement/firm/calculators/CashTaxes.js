...
const CashTaxes = () => {
  ...
  const [changesInAccruedOrDeferredTaxes, setChangesInAccruedOrDeferredTaxes] =
    useState('');
  const [currency, setCurrency] = useState('ZAR');
  const [solution, setSolution] = useState(0);
  //variables
  const variableName1 = 'Income Taxes';
  const variableName2 = 'Changes in Accrued or Deferred Taxes';
  ...
  return (
    <div className='calculator'>
      ...
      />
      <Input
        stateValue={changesInAccruedOrDeferredTaxes}
        onChangeHandler={onChangeHandler}
        um={currency}
        name={variableName2}
        type={type}
