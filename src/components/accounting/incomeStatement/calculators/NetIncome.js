...
import incomeStatement from '../../../../libs/formulae/accounting/incomeStatement';
//components
...
const NetIncome = () => {
  ...
  const [earningsBeforeTaxes, setEarningsBeforeTaxes] = useState('');
  const [corporateTaxes, setCorporateTaxes] = useState('');
  const [currency, setCurrency] = useState('ZAR');
  const [solution, setSolution] = useState(0);
  //variables
  const variableName1 = 'Earnings before Taxes';
  const variableName2 = 'Corporate Taxes';
  ...
  const onChangeHandler = (variableName, value) => {
    if (variableName === variableName1) {
      setEarningsBeforeTaxes(value);
    }
    if (variableName === variableName2) {
      setCorporateTaxes(value);
    }
  };
  const onACHandler = (variableName, value) => {
    setEarningsBeforeTaxes('');
    setCorporateTaxes('');
    setSolution(0);
  };
  const onSolveHandler = () => {
    const solution = new incomeStatement().netIncome(
      earningsBeforeTaxes,
      corporateTaxes,
    );
  ...
  return (
    <div className='calculator'>
      ...
      <Input
        stateValue={earningsBeforeTaxes}
